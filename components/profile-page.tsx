  ...
  const fetchUserPlanData = async () => {
    try {
      ...
      const dummyData: UserPlanData = {
        ...
        usage: {
          monthlyConsultations: 15,
          consultationLimit: 50,
          nextBillingDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
        }
      }
      
      setUserPlanData(dummyData)
      setFormData({
      ...
      <div className="container mx-auto px-4 max-w-6xl">
        <div className="mb-8">
          ...
          <div className="mt-2 p-2 bg-yellow-100 border border-yellow-400 rounded">
            <p className="text-yellow-800 text-sm">🧪 テストモード: 認証なしで表示中</p>
          </div>
        </div>
