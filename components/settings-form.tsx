    ...
    try {
      ...
      if (result.success) {
        ...
        setFormData({
          ...
          averageSpend: storeSettings.averageSpend ? storeSettings.averageSpend.toString() : "",
          seats: storeSettings.seats ? storeSettings.seats.toString() : "",
          businessHours: storeSettings.businessHours || "",
          challenges: storeSettings.challenges || [],
        })
        setIsFirstTime(storeSettings.isFirstTime)
        setLastUpdated(storeSettings.lastUpdated)
        
        if (storeSettings.isFirstTime) {
          toast.info("初回設定です。店舗情報を入力してAI分析の精度を向上させましょう！")
        }
      } else {
        setError(result.error || '設定の読み込みに失敗しました')
