                                ...
                                <div className="space-y-6">
                                  ...
                                  <div className="bg-gray-50 p-6 rounded-lg">
                                    ...
                                    {selectedAnalysis.aiAnalysis ? (
                                      ...
                                      <div className="text-center py-6 border-2 border-dashed border-gray-300 rounded-lg">
                                        ...
                                        <h5 className="text-lg font-medium text-gray-900 mb-2">AI分析結果はプレミアム機能です</h5>
                                        <p className="text-gray-600 mb-4">
                                          フリープランでは基本的な分析データのみご利用いただけます。<br />
                                          より詳細なAI分析をご希望の場合は、プランをアップグレードしてください。
                                        </p>
                                  ...
                                  {selectedAnalysis.analysisData ? (
                                    <div className="space-y-6">
                                      ...
                                      {/* 日別売上データ */}
                                      {selectedAnalysis.analysisData.dailySales && selectedAnalysis.analysisData.dailySales.length > 0 && (
                                        <div className="bg-white p-4 rounded-lg border border-gray-200">
                                          <h5 className="font-medium text-gray-900 mb-3">日別売上推移</h5>
                                          <div className="space-y-2 max-h-40 overflow-y-auto">
                                            {selectedAnalysis.analysisData.dailySales.slice(0, 10).map((item, index) => (
                                              <div key={index} className="flex justify-between items-center text-sm">
                                                <span className="text-gray-600">{item.date}</span>
                                                <span className="font-medium text-gray-900">{formatCurrency(item.sales)}</span>
                                              </div>
                                            ))}
                                          </div>
                                          {selectedAnalysis.analysisData.dailySales.length > 10 && (
                                            <p className="text-xs text-gray-500 mt-2">
                                              他 {selectedAnalysis.analysisData.dailySales.length - 10} 件のデータ
                                            </p>
                                          )}
                                        </div>
                                      )}
                                      ...
                                      {/* メニュー別売上データ */}
                                      {selectedAnalysis.analysisData.menuSales && selectedAnalysis.analysisData.menuSales.length > 0 && (
                                        <div className="bg-white p-4 rounded-lg border border-gray-200">
                                          <h5 className="font-medium text-gray-900 mb-3">メニュー別売上</h5>
                                          <div className="space-y-2 max-h-40 overflow-y-auto">
                                            {selectedAnalysis.analysisData.menuSales.slice(0, 10).map((item, index) => (
                                              <div key={index} className="flex justify-between items-center text-sm">
                                                ...
                                                <div className="text-right">
                                                  ...
                                                  <div className="text-xs text-gray-500">{item.value}個</div>
                                                </div>
                                              </div>
                                            ))}
                                          </div>
                                          {selectedAnalysis.analysisData.menuSales.length > 10 && (
                                            <p className="text-xs text-gray-500 mt-2">
                                              他 {selectedAnalysis.analysisData.menuSales.length - 10} 件のメニュー
                                            </p>
                                          )}
                                        </div>
                                      )}
                                      ...
                                      {/* 天気別売上データ */}
                                      {selectedAnalysis.analysisData.weatherSales && selectedAnalysis.analysisData.weatherSales.length > 0 && (
                                        <div className="bg-white p-4 rounded-lg border border-gray-200">
                                          <h5 className="font-medium text-gray-900 mb-3">天気別売上</h5>
                                          <div className="space-y-2">
                                            {selectedAnalysis.analysisData.weatherSales.map((item, index) => (
                                              <div key={index} className="flex justify-between items-center text-sm">
                                                <span className="text-gray-600">{item.weather}</span>
                                                <span className="font-medium text-gray-900">{formatCurrency(item.sales)}</span>
                                              </div>
                                            ))}
                                          </div>
                                    ...
                                    <div className="text-center py-8 bg-gray-50 rounded-lg">
                                      <FileText className="w-12 h-12 text-gray-400 mx-auto mb-4" />
                                      <h5 className="text-lg font-medium text-gray-900 mb-2">分析データを準備中</h5>
                                      <p className="text-gray-600">
                                        データの処理が完了していないか、分析結果の生成中です。<br />
                                        しばらく時間をおいてから再度ご確認ください。
                                      </p>
                                    </div>
                                  )}
                                </div>
